...
const preformatMakeCredReq = (makeCredRequest) => {
    ...
    if (makeCredRequest.excludeCredentials) {
        makeCredRequest.excludeCredentials = makeCredRequest.excludeCredentials.map((exCred) => {
...
const transformGetAssertRes = (assertRes) => {
    return {
        ...
        response: {
            ...
            userHandle: assertRes.response.userHandle ? bufferEncode(assertRes.response.userHandle) : null,
        },
        type: assertRes.type,
...
const Passkey = () => {
    ...
    const handleRegister = async () => {
        ...
        try {
            ...
            const { data: options } = await axios.post("http://localhost:3000/webauth/register", { username });
            ...
            const verifyResp = await axios.post("http://localhost:3000/webauth/verify-registration", {
                username,
                credential: credResponse,
            });
            setMessage(verifyResp.data.success ? "Registration successful!" : "Registration failed.");
        } catch (error) {
            console.error(error);
    ...
    const handleLogin = async () => {
        ...
        try {
            ...
            const { data: options } = await axios.post("http://localhost:3000/webauth/login", { username });
            ...
            const verifyResp = await axios.post("http://localhost:3000/webauth/verify-login", {
                username,
                assertion: assertionResponse,
            });
            setMessage(verifyResp.data.success ? "Login successful!" : "Login failed.");
        } catch (error) {
            console.error(error);
            setMessage(`Error: ${error.response?.data?.error || error.message}`);
        }
    ...
    return (
        <div className="min-h-screen flex flex-col items-center justify-center bg-gray-100 p-4">
            <div className="bg-white shadow-md rounded px-8 pt-6 pb-8 mb-4 w-full max-w-sm">
                <h1 className="text-xl font-bold mb-4 text-center">WebAuthn Passkey Demo</h1>
                <input
                    type="text"
