    ...
    <>
      ...
      >
        <div className="max-w-7xl mx-auto px-6 flex items-center justify-between">
                ...
                <div className="absolute right-0 mt-2 bg-gray-900 border border-gray-700 shadow-lg rounded-lg py-2 w-44 z-50 text-sm">
                  {user ? (
                    ...
                    <>
                      <Link
                        to="/login"
                        className="block px-4 py-2 text-cyan-400 hover:bg-gray-800 transition"
          ...
          >
            <span
              className={`block h-0.5 w-6 bg-gradient-to-r from-cyan-400 to-blue-500 transition-all duration-300 ${isMenuOpen ? 'transform rotate-45 translate-y-2' : ''}`}
            ></span>
            <span
              className={`block h-0.5 w-6 bg-gradient-to-r from-cyan-400 to-blue-500 transition-all duration-300 ${isMenuOpen ? 'opacity-0' : 'opacity-100'}`}
            ></span>
            <span
              className={`block h-0.5 w-6 bg-gradient-to-r from-cyan-400 to-blue-500 transition-all duration-300 ${isMenuOpen ? 'transform -rotate-45 -translate-y-2' : ''}`}
            ></span>
            ...
            <ul className="flex flex-col items-center py-4 space-y-4">
              ...
              <li className="pt-4">
                <div className="flex flex-col items-center">
                  ...
                  {user ? (
                    <>
                      <span className="text-cyan-400 font-semibold mb-1">
                        Hi, {user.fullName.split(' ')[0]}
                      </span>
                      <button
                        onClick={handleLogout}
